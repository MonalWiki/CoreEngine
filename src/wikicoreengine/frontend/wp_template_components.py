...
def render_nav_bar():
    with oj.uictx("topper") as navbarCtx:
        top_level_nav = oj.PC.StackH(childs = [oj.PC.Span(text="Navigation", twsty_tags=[fw.bold]), 
                                                oj.AC.A(key="history", href="#", text="History"),
                                                  oj.AC.A(key="index", href="#", text="Index"),
                                                  oj.AC.A(key="tags", href="#", text="Tags"),
                                                  oj.AC.A(key="user", href="#", text="User"),
                                              ]
                                     )
        
        
        page_trail = oj.PC.StackH(childs = [oj.PC.Span(text="Page Trail", twsty_tags =[fw.bold])
                                              ]
                                  )
...
def render_footer():
    with oj.uictx("footer") as footerbodyCtx:
        ...
        linkouthref = oj.PC.StackG( num_cols=3, childs = [
            ...
            oj.AC.A(key="ShreeLabs", text="Developed by Shree Labs ", href="#")
        ])
...
#                 oj.Container_("tlc", cgens= [tlctx.topper.panel,
#                                              _ictx.body.panel,
#                                              tlctx.footer.panel],
#                               pcp=[H/screen]
#                               )
#                 wp_ = oj.WebPage_(page_key,
#                                   cgens = [_ictx.tlc],
#                                   title=title,
#                                   WPtype=ojr.WebPage,
