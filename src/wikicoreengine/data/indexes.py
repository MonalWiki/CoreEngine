...
from ..constant_keys import (LATEST_REVS,
                             ALL_REVS,
                             NAMESPACE,
                             NAME,
                             CONTENTTYPE,
...
from .. import wikicfg
import time
from ..needs_home import WikiItem, IndexQueryAnswer
from ..contenttypes import  (NonExistent as Content_NonExistent,
                             Markdown as Content_Markdown
                             )
...
class Revision(NamedTuple):
    ...
    def create(idxitem, revid):
        ...
        print (doc)
        storage_revid = doc.get(REVID)
        assert namespace == ''
        meta, data = storage_routing.retrieve(namespace, storage_revid)
        revision = Revision(idxitem, revid, meta, data)
        return revision 
...
def indexes(storage_base_dir):
    ...
    def store_revision(indexItem:IndexQueryAnswer,
                       meta,
                       data,
                       overwrite=False,
        ...
        newIdxItem = IndexQueryAnswer(indexItem.query, current)
    ...
    def query(**query_params):
        ...
        latest_doc = document_search(**query_params)
        ...
        return IndexQueryAnswer(query_params, latest_doc)

    def save(wikiItem: WikiItem,
             meta,
             data=None,
             names=None,
             action = ACTION_SAVE,
        ...
        indexItem = query(**wikiItem.fqcn.query())
        
        currentrev = None
        contenttype_current = None
        #SelfNOte: don't call Revision if indexItem.current doest have a revid
        if indexItem.answer and indexItem.answer.get(REVID, None):
        ...
        # Unlike Moin, we don't abide to BytesIO business
        # if isinstance(data, bytes):
        #     data = BytesIO(data)
                    
        new_revision = store_revision(indexItem,
                                      meta,
                                      data,
                                      overwrite=overwrite,
                                      action=str(action),
...
def get_storage_revision(fqcn:Name.CompositeName, itemtype=None, contenttype=None, rev_id=CURRENT):
    ...
    # there are two routes to arrive here
    #1. via endpoint_wikiItem --> item.WikiItem::create --> fqcn is of type CompositeName...which has query member function
    
    #2. actions.py --> WikiItem.create --> wi.rev.idxitem 
